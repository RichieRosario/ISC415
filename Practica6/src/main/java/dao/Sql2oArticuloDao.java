...
//
    ...
    public List<Articulo> getArticles(int current_page){
        org.hibernate.Session session = HibernateUtil.openSession();
        javax.persistence.Query query = session.createQuery("From Articulo f ORDER BY f.fecha DESC");
        query.setFirstResult((current_page-1)*pageSize);
