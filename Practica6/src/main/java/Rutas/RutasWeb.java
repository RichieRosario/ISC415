...
//        usuarioDao.add(usuarioPorDefecto);
    ...
    //                articuloDao.add(articuloPrueba);
        ...
        get("/page/:numpage", (request, response) -> {
            ...
            Integer page_count = new Integer((int) articuloDao.getPageCount());

            if( numpage > page_count ){
                response.redirect("/page/" + page_count.toString() );
                return null;
            }

            List<Map<String, String> > articulos = new ArrayList<>();
            Long index = 0L;

            for(Articulo art : articuloDao.getArticles(numpage)){
        ...
        get("/", (request, response) -> {
            ...
            if(autenticado == true) {
                ...
                List<Articulo> articulos = articuloDao.getArticles(1) ;
                attributes.put("autor", autor);
                attributes.put("usuariodentro",user.getNombre());
                attributes.put("admin", admin);
                attributes.put("autenticado", autenticado);
                attributes.put("current_page", 1);
                attributes.put("maxpage", cntpage);
                attributes.put("articulos", articulos);
                attributes.put("pages", articuloDao.getPageCount());
            }
